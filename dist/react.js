...
validator:function validator(value){return['number','boolean'].includes(value);}}},data:function data(){return{selected:null};},created:function created(){this.init();// 监听LEDAP Model的事件，当属性变更时强制更新
if(this.model&&this.model.on){this.model.on(ledap.Model.EVENT_LOAD,this.handleModelChange.bind(this));}},beforeDestroy:function beforeDestroy(){// 清理事件监听
if(this.model&&this.model.off){this.model.off(ledap.Model.EVENT_LOAD,this.handleModelChange.bind(this));}},methods:{init:function init(){// 确保model[attr]有初始值
