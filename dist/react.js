...
onFinish===null||onFinish===void 0||onFinish(data,model.getFirstError());}return/*#__PURE__*/external_react_default().createElement(contexts_FormContext.Provider,{value:{getValue:getValue,setValue:setValue,updateView:updateView,model:model}},/*#__PURE__*/external_react_default().createElement(external_antd_.Form,_extends({className:classnames_default()(form_module_1,className,inline&&"inline"),onFinish:_onFinish},reset),props.children));}
...
var res=beforeUpload===null||beforeUpload===void 0?void 0:beforeUpload(file,fileList);if(res!==null&&res!==void 0&&res.then&&typeof res.then=="function"){var _res$then$catch,_res$then;(_res$then$catch=(_res$then=res.then(function(promiseResule){if(promiseResule!==false){// console.log("call _addFile", promiseResule);
_addFile(file);}}))["catch"])===null||_res$then$catch===void 0||_res$then$catch.call(_res$then,function(){});}else if(res===false){// 值类型
return false;}else{// console.log("call _addFile res", res);
_addFile(file);}}else{_addFile(file);}// 自定义上传
