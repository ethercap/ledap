...
total:pager.totalCount,showQuickJumper:true,showSizeChanger:false},paginationProp);var onRowSelectionChanged=function onRowSelectionChanged(selectedRowKeys,selectedRows){for(var i=0;i<data.length;i++){var model=data[i];if(selectedRowKeys.indexOf(model[rowKey])>-1){model.is_checked=true;}else{if(model.is_checked===true){model.is_checked=undefined;delete model.is_checked;}}}onSelectionChanged&&onSelectionChanged(selectedRowKeys,selectedRows);};var rowSelection=useSelection?Table_objectSpread({onChange:onRowSelectionChanged},rowSelectionProps||{}):rowSelectionProps;return/*#__PURE__*/external_react_namespaceObject["default"].createElement(external_antd_namespaceObject.Table,_extends({rowKey:rowKey,rowSelection:rowSelection,columns:antColumns,onChange:_onChange,dataSource:data,loading:isLoading,pagination:pagination},reset));}/**
