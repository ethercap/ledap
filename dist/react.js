...
function Detail(props){var model=props.model,columns=props.columns,_props$title=props.title,title=_props$title===void 0?null:_props$title;var _useState=(0,external_react_namespaceObject.useState)(Column.normalizeColumns(columns)),_useState2=_slicedToArray(_useState,2),ledapColumns=_useState2[0],setLedapColumns=_useState2[1];var _useState3=(0,external_react_namespaceObject.useState)(false),_useState4=_slicedToArray(_useState3,2),setBool=_useState4[1];var _updateView=function _updateView(){setBool(function(b){return!b;});};(0,external_react_namespaceObject.useEffect)(function(){setLedapColumns(Column.normalizeColumns(columns));},[columns]);var antColumns=Detail_getAntColumns(ledapColumns,model);return/*#__PURE__*/external_react_namespaceObject["default"].createElement(external_antd_namespaceObject.Descriptions,{title:title,items:antColumns});}function Detail_getAntColumns(ledapColumns,model){var targetColumns=[];var _loop=function _loop(){var column=ledapColumns[i];var label=column.label,value=column.value,attribute=column.attribute,visible=column.visible;// 不可见
if(visible==false){return 1;// continue
}var antdColumn={label:Detail_getTableTitle(column)};if(attribute){antdColumn.key=attribute;}if(typeof value=="function"){antdColumn.children=function(model){return value(model);};}else{antdColumn.children="".concat(model[attribute]);}targetColumns.push(antdColumn);};for(var i=0;i<ledapColumns.length;i++){if(_loop())continue;}return targetColumns;}function Detail_getTableTitle(column){var labelFormat=column.labelFormat,label=column.label,attribute=column.attribute;if(typeof label=="function"){if(labelFormat=="html"){return/*#__PURE__*/external_react_namespaceObject["default"].createElement("span",{dangerouslySetInnerHTML:{__html:label()}});}return label();}return label||attribute;}
