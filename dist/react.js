...
function HttpModel_callSuper(t,o,e){return o=_getPrototypeOf(o),_possibleConstructorReturn(t,HttpModel_isNativeReflectConstruct()?Reflect.construct(o,e||[],_getPrototypeOf(t).constructor):o.apply(t,e));}function HttpModel_isNativeReflectConstruct(){try{var t=!Boolean.prototype.valueOf.call(Reflect.construct(Boolean,[],function(){}));}catch(t){}return(HttpModel_isNativeReflectConstruct=function _isNativeReflectConstruct(){return!!t;})();}var HttpModel=/*#__PURE__*/function(_Model){function HttpModel(){var _this;var config=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};_classCallCheck(this,HttpModel);_this=HttpModel_callSuper(this,HttpModel);_defineProperty(_this,"_primaryKey",'id');_defineProperty(_this,"_basePath",'');_defineProperty(_this,"_viewPath",'/view');_defineProperty(_this,"_createPath",'/create');_defineProperty(_this,"_updatePath",'/update');_defineProperty(_this,"_deletePath",'/delete');_defineProperty(_this,"_keyMap",{httpRequest:'_httpRequest',primaryKey:'_primaryKey',basePath:'_basePath',viewPath:'_viewPath',createPath:'_createPath',updatePath:'_updatePath',deletePath:'_deletePath'});var keyMap=_this._keyMap;Object.keys(keyMap).forEach(function(key){if(config.hasOwnProperty(key)){_this[keyMap[key]]=config[key];}});if(!_this._httpRequest){throw new Error('httpRequest必须配置');}if(!_this._basePath){throw new Error('basePath必须配置');}return _this;}_inherits(HttpModel,_Model);return _createClass(HttpModel,[{key:"getConfig",value:function getConfig(){var _this2=this;var config={};var keyMap=this._keyMap;Object.keys(keyMap).forEach(function(key){if(_this2[keyMap[key]]){config[key]=_this2[keyMap[key]];}});return config;}},{key:"createUrl",get:function get(){return this._basePath+this._createPath;}},{key:"viewUrl",get:function get(){return this._basePath+this._viewPath;}},{key:"updateUrl",get:function get(){return this._basePath+this._updatePath;}},{key:"deleteUrl",get:function get(){return this._basePath+this._deletePath;}// 找到相关的参数
},{key:"getRequestData",value:function getRequestData(){var _this3=this;var excludePrimaryKey=arguments.length>0&&arguments[0]!==undefined?arguments[0]:false;var data={};Object.keys(this).forEach(function(key){if(_this3.isPrivateKey(key)){return;}data[key]=_this3[key];});if(excludePrimaryKey){delete data[this._primaryKey];}return data;}},{key:"getChangedRequestData",value:function getChangedRequestData(){var _this4=this;var changedData=this.getChangeData();var data={};Object.keys(changedData).forEach(function(key){if(_this4.isPrivateKey(key)){return;}data[key]=changedData[key];});return data;}},{key:"getModel",value:function getModel(){var _this5=this;var id=arguments.length>0&&arguments[0]!==undefined?arguments[0]:0;var params=arguments.length>1&&arguments[1]!==undefined?arguments[1]:{};return new Promise(function(resolve,reject){_this5._httpRequest({url:_this5.viewUrl+'?'+_this5._primaryKey+'='+id,params:params,method:'GET'},function(data){_this5.load(data.data);resolve({model:_this5,data:data});},function(error){reject(error);});});}},{key:"process",value:function process(httpOptions,resolve,reject){var _this6=this;this._httpRequest(httpOptions,function(data){_this6.load(data.data);_this6.sync();resolve({model:_this6,data:data});},function(error){reject(error);});}// 更新model
},{key:"updateModel",value:function updateModel(){var _this7=this;var dirtyObject=this.getChangedRequestData();return new Promise(function(resolve,reject){if(!_this7[_this7._primaryKey]){return reject(new Error('primaryKey不能为空'));}if(dirtyObject==null||Object.keys(dirtyObject).length===0){return resolve({model:_this7,data:{}});}if(dirtyObject.hasOwnProperty(_this7._primaryKey)){return reject(new Error('不允许更改primaryKey'));}_this7.process({url:_this7.updateUrl+'?'+_this7._primaryKey+'='+_this7[_this7._primaryKey],method:'POST',data:dirtyObject},resolve,reject);});}// 创建新model
},{key:"createModel",value:function createModel(){var _this8=this;var data=this.getRequestData(true);return new Promise(function(resolve,reject){_this8.process({url:_this8.createUrl,method:'POST',data:data},resolve,reject);});}// 删除当前model
},{key:"deleteModel",value:function deleteModel(){var _this9=this;return new Promise(function(resolve,reject){if(!_this9[_this9._primaryKey]){return reject(new Error('primaryKey不能为空'));}_this9.process({url:_this9.deleteUrl+'?'+_this9._primaryKey+'='+_this9[_this9._primaryKey],method:'POST',data:{}},resolve,reject);});}// 保存model，根据是否有primaryKey判断是更新还是创建
},{key:"save",value:function save(type){if(type==="create"){return this.createModel();}else if(type==="update"){return this.updateModel();}else{if(this[this._primaryKey]){return this.updateModel();}else{return this.createModel();}}}}],[{key:"find",value:function find(){var config=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};var params=arguments.length>1&&arguments[1]!==undefined?arguments[1]:{};var model=new this(config);if(params.hasOwnProperty('withConfig')){params['withConfig']=!!params['withConfig'];}return model.getModel(params[model._primaryKey]||0,params);}}]);}(Model_Model);
;// ./src/base/index.ts
...
inputChange:function inputChange(e){this.value=e.target.value;if(this.value){this.request(_defineProperty({},this.paramName,this.value));}if(this.inputListeners.input){this.inputListeners.input(e);}},focusChange:function focusChange(e){if(this.inputListeners.focus){this.inputListeners.focus(e);}},blurChange:function blurChange(e){if(this.inputListeners.blur){this.inputListeners.blur(e);}},// 选择model
...
var select2_input=cloneDeep_default()(searchinput);/* harmony default export */ const select2 = (merge_default()(select2_input,{name:'select2',props:{keyName:{type:String,"default":'id'},multiple:{type:Boolean,"default":false},placeholder:{type:String,"default":''},webFocusSearch:{type:Boolean,"default":false}},mounted:function mounted(){var _this2=this;if(this.filter===null){this.filter=function(model,index,collection){var keyword=String(_this2.value||'').toLowerCase();if(!keyword){return true;}var text=String(model[_this2.itemName]||'').toLowerCase();return text.indexOf(keyword)>-1;};}this.init();},data:function data(){return{showError:this.model.getFirstError(this.attr),isFocus:false,isHide:false,// input的值
value:'',models:this.dataProvider.models||[],filter:this.dataFilter,selected:{}};},computed:{showList:function showList(){return this.isFocus;},listeners:function listeners(){var _this3=this;return assign_default()({},this.$listeners,{input:function input(e){_this3.inputChange(e);},focus:function focus(e){_this3.isFocus=true;_this3.focusChange(e);if(!_this3.isWebDp()||_this3.webFocusSearch){_this3.request(_defineProperty(_defineProperty({},_this3.keyName,[]),_this3.paramName,_this3.value));}},blur:function blur(e){_this3.isHide=true;setTimeout(function(){_this3.isHide=false;_this3.isFocus=false;},_this3.delay);_this3.blurChange(e);}});},// 这里的formValue是表单里提交给后端的值
formValue:function formValue(){return this.model[this.attr];}},watch:{formValue:function formValue(newValue,oldValue){this.init();},selected:function selected(value){var keys=Object.keys(value);if(this.multiple){// 为了防止死循环，只有当值不同的时候才进行赋值
if(!isEqual_default()(this.model[this.attr],keys))this.model[this.attr]=keys;}else{this.model[this.attr]=keys.length>0?keys[0]:'';this.value=keys.length>0?value[keys[0]][this.itemName]:'';}}},methods:{isWebDp:function isWebDp(){return this.dataProvider.refresh&&typeof this.dataProvider.refresh==='function';},init:function init(){// 如果没有selected,先请求
if(this.model[this.attr]&&isEmpty_default()(this.selected)){return this.request(_defineProperty(_defineProperty({},this.keyName,this.model[this.attr]),this.paramName,this.value),this.syncSelected);}this.syncSelected();},request:function request(params){var _this4=this;var callback=arguments.length>1&&arguments[1]!==undefined?arguments[1]:function(key){};if(this.isWebDp()){this.dataProvider.callback=function(data){_this4.models=_this4.dataProvider.models;callback(_this4.models);};this.dataProvider.setParams(params);}else{if(!this.value){this.models=this.dataProvider.models||[];callback(this.models);return;}this.models=filter_default()(this.dataProvider.models||[],this.filter);callback(this.models);}},focusChange:function focusChange(e){if(this.$listeners.focus){this.$listeners.focus(e);}},syncSelected:function syncSelected(){if(this.multiple&&_typeof(this.model[this.attr])!='object'){return this.model[this.attr]=[this.model[this.attr]];}var selected={};var _this=this;Object.keys(this.models).forEach(function(index){var model=_this.models[index];if(_this.multiple){if(ArrayHelper.hasKey(_this.model[_this.attr],model[_this.keyName])){selected[model[_this.keyName]]=model;}}else if(String(model[_this.keyName])===String(_this.model[_this.attr])){selected[model[_this.keyName]]=model;}});return this.selected=selected;},inputChange:function inputChange(e){this.value=e.target.value;if(!this.isWebDp()||this.value){this.request(_defineProperty(_defineProperty({},this.keyName,[]),this.paramName,this.value));}if(this.$listeners.input){this.$listeners.input(null);}},blurChange:function blurChange(e){if(!this.multiple){var index=this.model[this.attr];if(this.selected[index]){this.value=this.selected[index][this.itemName];}else{this.value='';}}if(this.$listeners.blur){this.$listeners.blur(e);}},// 选择model
choose:function choose(model,index,e){var _this5=this;if(typeof model[this.itemName]!=='undefined'){// 已选中
if(this.selected.hasOwnProperty(model[this.keyName])){this.$delete(this.selected,model[this.keyName]);this.value='';}else{if(this.multiple){this.value='';this.listeners.focus();if(this.$refs.input){this.$refs.input.focus();}}else{this.value=model[this.itemName];this.selected={};}this.$set(this.selected,model[this.keyName],model);}setTimeout(function(){_this5.request(_defineProperty(_defineProperty({},_this5.keyName,[]),_this5.paramName,_this5.value));},this.delay);}this.$emit('choose',model,index,e);},clear:function clear(){this.selected={};this.value='';if(!this.isWebDp()){this.request(_defineProperty(_defineProperty({},this.keyName,[]),this.paramName,this.value));}this.$emit('clear');}},template:"<div style=\"position: relative;\">\n    <span>\n        <span v-if=\"multiple\" v-for=\"model,key in selected\" :key=\"key\" @click=\"choose(model, key, $event)\">{{model[itemName]}}</span>\n        <input ref=\"input\" :name=\"attr\" :value=\"value\" :placeholder=\"placeholder || model.getAttributeHint(attr)\" v-on=\"listeners\" autocomplete=\"off\" v-bind=\"$attrs\">\n        <span v-if=\"!multiple && value\" @click=\"clear\">X</span>\n    </span>\n    <ul v-show=\"showList\" style=\"position: absolute;\" :style=\"{opacity: isHide ? 0 : 1}\">\n        <div v-if=\"dataProvider.isLoading\">\u52A0\u8F7D\u4E2D</div>\n        <template v-else>\n            <template v-if=\"models.length\">\n                <li v-for=\"(model, index) in models\" @click=\"choose(model, index, $event)\">\n                    <slot name=\"tab\" :model=\"model\" :index=\"index\" :isActive=\"selected.hasOwnProperty(model[keyName])\">{{model[itemName]}}</slot>\n                </li>\n            </template>\n            <div v-else>\u65E0\u6570\u636E</div>\n        </template>\n    </ul>\n</div>"}));
...
},{key:"getAttributeLabel",value:function getAttributeLabel(attribute){var defaultValue=arguments.length>1&&arguments[1]!==undefined?arguments[1]:'';var _attrLabels=this.attributeLabels();return _attrLabels[attribute]||defaultValue||attribute;}},{key:"getConfig",value:function getConfig(){return{};}},{key:"clone",value:function clone(){var _this7=this;var data=arguments.length>0&&arguments[0]!==undefined?arguments[0]:null;var model=new this.constructor(this.getConfig());model._attrRules=cloneDeep_default()(this._attrRules);model._attrLabels=cloneDeep_default()(this._attrLabels);model._attrHints=cloneDeep_default()(this._attrHints);model._errors=cloneDeep_default()(this._errors);if(!data){data={};Object.keys(this).forEach(function(key){if(!_this7.isPrivateKey(key)){data[key]=cloneDeep_default()(_this7[key]);}});}model.load(data);return model;}// reset
},{key:"reset",value:function reset(){var _this8=this;Object.keys(this._old).forEach(function(key){_this8[key]=_this8._old[key];});}},{key:"sync",value:function sync(){var _this9=this;Object.keys(this._old).forEach(function(key){_this9._old[key]=cloneDeep_default()(_this9[key]);});}},{key:"getChangeData",value:function getChangeData(){var _this10=this;var dirtyObject={};Object.keys(this._old).forEach(function(key){if(!isEqual_default()(_this10[key],_this10._old[key])){dirtyObject[key]=_this10[key];}});return dirtyObject;}}]);}(BaseObject);_defineProperty(Model_Model,"SCENARIO_DEFAULT",'default');_defineProperty(Model_Model,"EVENT_BEFORELOAD",'MODEL_BEFORE_LOAD');_defineProperty(Model_Model,"EVENT_LOAD",'MODEL_LOAD');_defineProperty(Model_Model,"EVENT_AFTERLOAD",'MODEL_AFTER_LOAD');_defineProperty(Model_Model,"EVENT_BEFORE_VALIDATE",'MODEL_BEFORE_VALIDATE');_defineProperty(Model_Model,"EVENT_AFTER_VALIDATE",'MODEL_AFTER_VALIDATE');
